...
export const updateStatsForAction = (item: FoodItem, action: 'consumed' | 'tossed' | 'composted'): UserStats => {
  ...
  if (!dailyLog) {
    dailyLog = { date: today, calories: 0, protein: 0, carbs: 0, fat: 0, itemsConsumed: 0, itemsWasted: 0 };
    current.dailyLogs.push(dailyLog);
  }
  ...
  if (action === 'consumed') {
    ...
    // Update daily log
    dailyLog.itemsConsumed += 1;
    dailyLog.calories += (item.calories || 0) * quantity;
    // Mock nutrients based on category
    if (item.category === 'Meat') dailyLog.protein += 20 * quantity;
    if (item.category === 'Dairy') { dailyLog.protein += 8 * quantity; dailyLog.fat += 10 * quantity; }
    if (item.category === 'Produce') dailyLog.carbs += 15 * quantity;
    if (item.category === 'Grains') dailyLog.carbs += 40 * quantity;
